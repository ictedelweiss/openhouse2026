...
export default function AdminDashboardPage() {
  ...
  // Tab Navigation
  const [activeTab, setActiveTab] = useState<'registrations' | 'schedules' | 'admins'>('registrations');
  ...
  const fetchAdmins = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}?action=get_admins`);
      const json = await res.json();
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`${API_BASE_URL}?action=create_admin`, {
        method: 'POST',
  ...
  const handleDeleteAdmin = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}?action=delete_admin`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const json = await res.json();
      alert(json.message);
      if (json.status === 'success') fetchAdmins();
    } catch (e) { console.error(e); }
  };

  const handleVerifyPayment = async (id: number, status: 'verified' | 'rejected') => {
    const confirmMsg = status === 'verified' ? 'Setujui pembayaran ini?' : 'Tolak pembayaran ini?';
    if (!confirm(confirmMsg)) return;

    try {
      const res = await fetch(`${API_BASE_URL}?action=verify_payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                            ...
                            <>
                              ...
                              {(!item.payment_status || item.payment_status === 'pending') && (
                                <div className="flex gap-1 mt-1">
                                  ...
                                  <button onClick={() => handleVerifyPayment(item.id, 'rejected')} className="flex-1 bg-rose-500 hover:bg-rose-600 text-white font-bold py-1 rounded text-[10px]">Tolak</button>
                                </div>
                              )}
                            </>
                          ) : (
